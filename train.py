...
def trainer(net, image_datasets, num_epoch, criterion, optimizer, scheduler, device, pin_memory, exp_path, writer):
    dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}
    class_names = image_datasets['train'].classes
    num_iter = {x: dataset_sizes[x]//C.get()['batch_size'] for x in ['train', 'val']}
    num_cycle = {x: int(num_iter[x] * C.get()['log_cycle']) for x in ['train', 'val']}
    logging.info(f'number of iter: {num_iter}')
    logging.info(f'iter cycle for tensorboard: {num_cycle}')
    logging.info('training start ......!')
    training_start_time = time.time()
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=C.get()['batch_size'], \
        ...
        for phase in ['train', 'val']:
            ...
            for i, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                if i % num_cycle[phase] == num_cycle[phase]-1:
                    ...
                    loss_iters = running_loss/((i+1)*C.get()['batch_size'])
                    acc_iters = running_corrects/((i+1)*C.get()['batch_size'])
